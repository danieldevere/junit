...
public class JunitApplicationTests {
	...
	public void addEmployeeThenSearch() {
		ResponseEntity<String> response1 = template.postForEntity("/employee", employee, String.class);
		assertThat(response1.getStatusCode()).isEqualTo(HttpStatus.OK);
		assertThat(response1.getBody()).isEqualTo("Added employee");
		ResponseEntity<Employee> response2 = template.getForEntity("/employee/" + employee.getId(), Employee.class);
	...
	public void addEmployeeThenUpdateThenSearch() throws Exception {
		...
		ResponseEntity<String> response2 = template.exchange("/employee", HttpMethod.PUT, request, String.class);
		assertThat(response2.getStatusCode()).isEqualTo(HttpStatus.OK);
		assertThat(response2.getBody()).isEqualTo("updated employee");
		ResponseEntity<Employee> response3 = template.getForEntity("/employee/" + employee.getId(), Employee.class);
		...
		assertThat(response3.getBody().getAnnualReview(2018).getPerformanceNotes().get(0).getNoteText()).isEqualTo("hello");
	}
	@DirtiesContext
	@Test
	public void addEmployeeThenDelete() {
		...
		assertThat(response1.getStatusCode()).isEqualTo(HttpStatus.OK);
		assertThat(response1.getBody()).isEqualTo("Added employee");
		ResponseEntity<String> response2 = template.exchange("/employee/" + employee.getId(), HttpMethod.DELETE, null, String.class);
		assertThat(response2.getStatusCode()).isEqualTo(HttpStatus.OK);
